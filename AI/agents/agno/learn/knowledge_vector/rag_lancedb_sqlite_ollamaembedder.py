...
db = SqliteDb(db_file="tmp/ollama_embedder.db")

# Initialize the Agent with various configurations including the knowledge base and storage
agent = Agent(
    # model=Cerebras(id="qwen-3-235b-a22b-instruct-2507"),
    session_id="ollama_embed_session",  # use any unique identifier to identify the run
    user_id="William",  # user identifier to identify the user
    model=model,
    knowledge=knowledge,
    db=db,
)
